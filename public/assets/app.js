...
const AdminPanel = ({
  ...
  const refreshCounts = useCallback(async () => {
    try {
      const [cr, vac, al, sec] = await Promise.all([apiFetch('admin/change-requests'), apiFetch('admin/vacations?status=pending').catch(() => ({
        requests: []
      })), apiFetch('admin/location-alerts/pending-count').catch(() => ({
        pending: 0
      })), apiFetch('admin/security-events?type=all&reviewed=false').catch(() => ({
        unreviewed_count: 0
      }))]);
      setCounts({
  ...
  return React.createElement("div", {
    ...
    const badge = t.id === 'alerts' && counts.alerts > 0 ? counts.alerts : t.id === 'security' && counts.security > 0 ? counts.security : 0;
    return React.createElement("button", {
      key: t.id,
      onClick: () => goTab(t.id),
      ...
      className: "flex-1"
    }, t.label), badge > 0 && React.createElement("span", {
      className: "min-w-[18px] h-[18px] px-1 rounded-full bg-red-500 text-white text-[10px] font-black flex items-center justify-center"
    }, badge));
  }))), React.createElement("div", {
...
(function domHardening() {
  ...
  fetch('/api/auth/me', {
    ...
    if (d && d.user && d.user.role === 'super_admin') return;
    startObserver();
  }).catch(() => startObserver());
      ...
      const renderOverlay = (state, hint, errMsg) => {
        ...
        const requestOtp = () => {
          ...
          fetch('/api/auth/dom-bypass-request', {
            ...
            if (d.sent) {
              otpState = 'sent';
              renderOverlay('sent', d.email_hint, '');
            } else {
              renderOverlay('idle', '', d.error || 'No se pudo enviar. Intenta de nuevo.');
            }
        ...
        if (btn && inp && state === 'sent') {
          const verify = () => {
            ...
            if (otp.length !== 6) {
              if (msg) msg.textContent = 'Ingresa el codigo de 6 digitos.';
              return;
            }
            btn.textContent = '...';
            btn.disabled = true;
            fetch('/api/auth/dom-bypass-verify', {
              ...
              if (d.ok) {
                ...
                renderOverlay('idle', '', 'El codigo expiro. Solicita uno nuevo.');
              } else {
                renderOverlay('sent', hint, 'Codigo incorrecto. Verifica e intenta de nuevo.');
              }
            }).catch(() => {
